...
function Login() {
  ...
  const submit = async (data) => {
    try {
      ...
      dispatch(setUser(res.data.user));  // ← triggers useEffect → navigate("/")
    } catch (error) {
    ...
    <div className="min-h-screen bg-linear-to-br from-slate-950 via-gray-900 to-indigo-950 flex items-center justify-center px-4">
      <div className="w-full max-w-md bg-white/5 backdrop-blur-lg border border-white/10 rounded-2xl shadow-2xl p-8">
        <div className="text-center mb-8">
          <img src={logo} alt="NovaChat Logo" className="w-24 h-24 mx-auto mb-4 rounded-full shadow-lg" />
          <h1 className="text-4xl font-bold text-purple-500">NovaChat</h1>
        ...
        <p className="text-center text-gray-400 mt-6">
          ...
          <span className="text-purple-400 cursor-pointer hover:text-purple-300" onClick={() => navigate("/signup")}>
            Signup
          </span>
        </p>
      </div>
